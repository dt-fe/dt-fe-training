...
class ComplexCard extends React.Component {
  ...
  complexCheck(e) {  //用来做复杂筛选框的选中
    ...
    // let ck = e.target, checkarr = this.state.check, checked = [], visib = []
    let ck = e.currentTarget, checkarr = this.state.check, checked = [], visib = []
    ...
    // if (ck.checked === true) {
    //   checkarr.push(ck.id);
    // } else if (checkarr.includes(ck.id)){
    //   let index = checkarr.indexOf(ck.id)
    //   checkarr.splice(index, 1)
    // }
    ck.id = ck.id.split("t")[1]
          ...
          this.state.detailAll.map((detail, i) => {
            return (
              <div className="card" key={detail.data.value === "404" ? 404 + i : detail.data.value}>
                ...
                <div id="cardbtw" >
                  <button id={"bt" + i} onClick={(e) => this.complexCheck(e)} style={{ backgroundColor: "transparent" }}>
